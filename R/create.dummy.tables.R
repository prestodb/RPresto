...
#' | 8 | decimal | DECIMAL | \[1.1, 2.2, null\] |
...
#' an `ARRAY`. Each `ROW` element has all 18 supported primitive data types.
#'
#' [create_array_of_rows_table()] creates a dummy table that has an `ARRAY(ROW)`
#' column that has 2 `ROW` elements, each containing all 18 supported primitive
#' data types.
#'
#' We construct the **array-of-rows** table by first creating a table with
#' a `ROW` type column that includes all 18 supported primitive data types. We
...
#' an `ARRAY`.
#'
#' [create_array_of_maps_table()] creates a dummy table that has 18 `ARRAY(MAP)`
